...
- [ ] **訂單詳情顯示停靠點**：`OrderDetailPage` 目前只有上車／下車兩點＋軌跡回放。
      要依序列出各停靠點（乘客標籤 A/B/C/D、上車／下車、地址、到達時間），
      並在 MapLibre 地圖上標出多個點（現行只畫軌跡線）。
- [ ] **訂單列表**：是否顯示「多乘客」標記／乘客數欄位（待產品定）。
- [ ] **報表**：多停靠點行程的里程／車資已由後端定格，admin 只呈現，**勿在前端算錢**（沿用既有原則）。

### 司機車輛資訊／寵物車清潔費（依賴後端 O1–O7）

- [ ] **司機列表／詳情顯示車種車牌**：`DriversPage`／`DriverDetailPage` 加欄位。
      車種為**選單值**（後端存 code：`sedan`／`suv`／`van7`／`accessible`／`pet`），
      admin 端要對應顯示名稱（轎車／休旅／七人座／無障礙／寵物用車）。
- [ ] **搜尋**：車牌納入司機搜尋條件（現行搜尋為姓名／電話／狀態）。
- [ ] **車種篩選**：依車種過濾司機（有了選單才做得到；自由文字時期做不到）。
- [ ] **寵物車清潔費設定** ✅ 定案（2026-07-16）：`FeeSettingsPage` 加「寵物車清潔費（%）」欄位，
      串後端 `pet_cleaning_fee_bps`（bps↔% 換算，比照既有「遺失物協尋處理費（%）」）。
      **前端要擋 0–30%**（後端有 DB CHECK ≤ 3000 bps 為最後防線），
      並用 tooltip 註明「完成行程當下快照，不影響歷史行程」。
- [ ] **報表清潔費呈現** ✅ 已拍板（2026-07-16）：**清潔費不計入抽成、不計入營業額**，
      報表**分項顯示**——日／月報表加「清潔費」欄（後端會在 F5/F6/F7 加分項欄位），
      營業額／手續費／應付總公司欄位的語意不變。CSV 匯出同步加欄。
      沿用既有原則：**金額由後端定格，admin 只呈現，勿在前端算錢**。
- [ ] **訂單顯示「乘客指定車種」** ✅ 定案（2026-07-16，主規格 P）：
      `rides.required_vehicle_type` 是清潔費的觸發依據，訂單詳情要看得到
      （否則客服無法解釋「為什麼這筆多收清潔費」）。
      與「司機當時的車輛快照」是**兩個不同欄位**，不要混淆：
      前者＝乘客要求什麼車，後者＝實際派來哪台車。
- [ ] **（可選）車種供給觀察**：若要支援「車隊目前有哪些車種可用」（主規格 P 風險 2），
      admin 的即時車隊頁是天然的呈現點（現行 marker popup 已有姓名／狀態／更新時間）。
- [ ] **訂單詳情顯示車輛快照**：後端 O7 會在 `rides` 落當時的車種車牌 →
      訂單詳情要顯示「當時搭的車」（而非司機現在的車），避免換車後對不上。
- [ ] **（可選）車輛審核**：若後端採「審核制」（主規格 O5 待拍板），
      admin 需要審核 UI（pending → approved），且要考慮 RBAC（誰能審）。
