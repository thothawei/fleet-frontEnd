...
> **💰 高優先（使用者要求 2026-07-15）：金額改用整數台幣（無小數）。** 台幣沒有小數點，目前
> `src/utils/money.ts` 顯示「分→NT$XX.XX（兩位小數）」會出現 `NT$21.03`、`NT$17.96` 這種不可支付金額。
> 待後端把車資/手續費/會費/處理費/報表改成整數元後，admin 這端要同步：`money.ts` 顯示與 CSV 不再有小數、
> 費率設定頁輸入單位、日/月報表、會費帳單、協尋處理費顯示全部整數化。**主規格與決策見**
